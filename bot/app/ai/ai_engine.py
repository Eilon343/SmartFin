...
def _build_prompt(user_input: str, categories: list[str]) -> str:
    ...
    return (
        f"Analyze this message and determine the user's financial intent. Return ONLY valid JSON.\n"
        ...
        f"Return one of these exact JSON shapes:\n\n"
        f"log_expense:\n"
        f"  {{\"intent\":\"log_expense\",\"amount\":55.0,\"currency\":\"ILS\",\"item\":\"shawarma\",\"category\":\"Food\"}}\n\n"
        f"log_income:\n"
        ...
        f"If the message has no financial meaning at all, return ERROR_UNSUPPORTED."
    )
